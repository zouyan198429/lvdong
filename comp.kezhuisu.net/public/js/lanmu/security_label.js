...
//批量删除
    ...
    $(document).on("click",".auto_create",function(){
        var index_query = layer.confirm('自动生成防伪标签,请联系客服人员为您生成！', {
            btn: ['确定'] //按钮
        }, function(){
            // operate_ajax('del',id);
...
function operate_ajax(operate_type,id){
    ...
    switch(operate_type)
    {
        case 'del'://删除
            operate_txt = "删除";
            data = {'id':id}
            ajax_url = SECURITY_LABEL_DEL_URL;// /pms/Supplier/ajax_del?operate_type=1
            break;
        case 'batch_del'://批量删除
            operate_txt = "批量删除";
            data = {'supplier_id':id}
...
(function() {
    ...
    document.write("   <td><%=item.security_label_num%><\/td>");
