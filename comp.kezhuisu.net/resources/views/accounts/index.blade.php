...
function operate_ajax(operate_type,id){
    ...
    switch(operate_type)
    {
        case 'del'://删除
            operate_txt = "删除";
            data = {'id':id}
            ajax_url = "{{ url('api/accounts/ajax_del') }}";// /pms/Supplier/ajax_del?operate_type=1
            break;
        case 'batch_del'://批量删除
            operate_txt = "批量删除";
            data = {'supplier_id':id}
...
<%for(var i = 0; i<data_list.length;i++){
    ...
    <tr>
        ...
        <td>
        <%if( account_status == 0){%>
            <span class="text-green"><%=item.account_statu_text%></span>
        <%}else{%>
            <span class="text-gray"><%=item.account_statu_text%></span>
        <%}%>
        </td>
        <td>
